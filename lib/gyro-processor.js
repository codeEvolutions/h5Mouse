...
class AbsoluteProcessor extends BasicProcessor{
    ...
    process(event){
        ...
        if(!gyro) return;
        if(this.last_gyro && gyro.gamma - this.last_gyro.gamma > 30) robot.mouseClick("right");
        else if (this.last_gyro && gyro.gamma - this.last_gyro.gamma < -30) robot.mouseClick("left");
        else{
...
class RelativeProcessor extends BasicProcessor{
    constructor(){
        ...
        this.startTimer=()=>{
            ...
            that.timer = setInterval(()=>{
                if(that.timer && new Date() - that.last_event_time > global.$setting.send_time_gap){
    ...
    process(event){
        var gyro = super.process(event);
        if(!gyro) return;
        if(this.initial_x === null)this.initial_x = gyro.alpha,this.initial_y = gyro.beta;
