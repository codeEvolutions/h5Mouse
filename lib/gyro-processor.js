...
class BasicProcessor {
  ...
  process (event) {
    ...
    if (gyro.alpha && gyro.beta && gyro.gamma) {
      ...
      // TODO: Recalculate gyro base on this.initialGyro.
      return gyro;
    } else return false;
  }
}
...
class AbsoluteProcessor extends BasicProcessor {
  ...
  process (event) {
    ...
    if (!gyro) return;
    if (this.last_gyro && gyro.gamma - this.last_gyro.gamma > 30) { robot.mouseClick('right'); } else if (this.last_gyro && gyro.gamma - this.last_gyro.gamma < -30) { robot.mouseClick('left'); } else {
